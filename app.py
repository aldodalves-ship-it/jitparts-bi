...
# ── Bootstrap de dados ───────────────────────────────────────────────
# Importado aqui para estar disponível antes do main()
from bootstrap import ensure_data_ready  # noqa: E402
...
def main() -> None:
    ...
    # ── Bootstrap: garante que os dados existem antes de carregar ────
    # Se dashboard_base_final.csv nao existe → executa pipeline de coleta
    # Se credenciais ausentes → exibe instrucoes e para
    if not ensure_data_ready(show_ui=True):
        return  # bootstrap exibiu mensagem de erro; nao prosseguir
    ...
    try:
        ...
        st.error(
            f"❌ Arquivo de dados nao encontrado: {exc}\n\n"
            "**Para resolver:**\n"
            "- Configure as credenciais ML em **Settings → Secrets** e recarregue a pagina\n"
            "- Ou faca upload dos arquivos CSV na pasta `data/` do repositorio"
        )
